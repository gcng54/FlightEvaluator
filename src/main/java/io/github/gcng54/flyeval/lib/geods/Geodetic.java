...
/**
    ...
    public record Geodetic(Angle lon, Angle lat, Length alt) implements ISpherical<Geodetic> {
        ...
        public Cartesian toENU(Cartesian ecefVector) {
            double lonRad = this.lon.inRadians();
            double latRad = this.lat.inRadians();
            ...
            // The rotation matrix from ECEF to ENU is the transpose of the ENU-to-ECEF
            // matrix.
            ...
            return new Cartesian(Length.fromMeter(east),
                    Length.fromMeter(north),
                    Length.fromMeter(up));
        }
